 ...
 * 객체 생성시 마지막에 호출되어 객체 필드 초기화를 담당.
 * 
 * return 타입이 없고 , 클래스명과 동일해야 한다.
 * 
 * - 기본 생성자 : 파라미터 생성자가 없다면 JVM에 의해 자동생성
 *                   파라미터 생성자가 하나라도 있다면, 자동생성 안됨 , 직접 작성해야함
 *                   
 * - 파라미터 생성자 : 필드값을 넘겨받아서 한번에 값세팅
 * 
 * 기본 생성자가 필요한 이유
 * 1. 상속시 자식클래스에서 부모 클래스의 기본 생성자를 호출
...
public class User {
	...
	public User(String userId , String password , Date regDate) {
		this(userId , password); // 다른 생성자 호출 = > 중복된 코드 제거
//		this.userId = userId;
//		this.password = password;
