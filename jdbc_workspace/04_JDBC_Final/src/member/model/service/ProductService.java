...
public class ProductService {
	...
	private ProductDao memberDao = new ProductDao();
	
	public List<Product> selectAll() {
		Connection conn = getConnection(); //static 메소드라서 글씨체가 다름 getConnection
		List<Product> list = memberDao.selectAll(conn);
	...
	*/
	...
	public List<Product> selectAll_() {
	...
	public int insertMember(Product member) {
		Connection conn = getConnection();
		int result = memberDao.insertProduct(conn,member);
		if(result > 0) 
				commit(conn);
		else 
				rollback(conn);
	...
	public Product selectOneMember(String memberId) {
		
		//1. Connection객체 생성
				Connection conn = getConnection();
				//2. dao에 Connection객체, memberId를 전달해서 Member객체를 리턴받음.
				Product m = memberDao.selectOneProduct(conn, memberId);
				//3. Connection 자원반납
				close(conn);
...
//				System.out.println("member@service="+m);
			...
			public List<Product> selectByName(String memberName) {
				Connection conn = getConnection();
				List<Product> list = memberDao.selectByName(conn, memberName);
				close(conn);
				return list;
			}
			...
			public Product selectOne(String memberId){
				Connection conn = getConnection();
				Product m = memberDao.selectOne(conn, memberId);
				close(conn);
				return m;
			}
